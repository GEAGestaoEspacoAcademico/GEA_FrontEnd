...
import type {
  ...
  AdicionarRecursoSalaResponse,
  AtualizarSalaRequest,
  AtulizarQuantidadeRecursoSalaRequest,
  BuscarRecomendacaoRequest,
  BuscarRecomendacaoResponse,
  BuscarRecursoSalaResponse,
  BuscarRecursoSalaResponseArray,
  BuscarSalaDisponivel,
  CriarSalaRequest,
  CriarSalaResponse,
...
export class SalaService {
  ...
  public editQuantidadeRecursoSala(
    salaId: number,
    recursoId: number,
    recurso: AtulizarQuantidadeRecursoSalaRequest,
  ): Observable<void> {
  ...
  public adicionarRecursoEmSala(
    salaId: number,
    recurso: AdicionarRecursoSalaRequest,
  ): Observable<AdicionarRecursoSalaResponse> {
    return this.http.post<AdicionarRecursoSalaResponse>(
      `${this.baseUrl}/${salaId}/recursos`,
      recurso,
    );
  }
  ...
  public getLaboratorios(): Observable<Sala[]> {
    return this.http
      .get<Sala[]>(this.baseUrl)
      .pipe(map((salas: Sala[]) => salas.filter((s) => this.isLaboratorio(s))));
  }

  private isLaboratorio(s: Sala): boolean {
    const textFields: string[] = [s.salaNome ?? ''];
